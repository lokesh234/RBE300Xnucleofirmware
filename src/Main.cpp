 ...
 * required to control the arm. Please, take some time to familiarize yourself
 * with the workflow of the program.
 *
 * IMPORTANT - this program has two distinct running modes:
 * **Dummy mode** to be used for firmware testing/debugging when no arm is
 * connected.
 * **Physical mode** to be used when the robotic arm is connected to the Nucleo
 * board.
 *
 * The running mode can be selected at compile time by commenting/uncommenting
 * the DUMMYMODE macro below.
 *
 */
#include "main.h"
#define DOFs 3 // this macro defines the number of joints of the robotic arm
//#define  DUMMYMODE   // this macro selects the running mode - see instructions
//above
...
float homePosition[3] = {-997.0, 2960.325, 2287.05};
...
int main() {
  ...
  for (int i = 0; i < DOFs; i++) // for each joint,
  {
...
#ifdef DUMMYMODE // if operating in Dummy Mode, set the initial encoder reading
                 // to zero
    pid[i]->ZeroPID();
    ...
    // !FIXME Do we need the following two instructions? I'm afraid this may
    // generate strange behaviors.
    if (pid[i]->GetPIDPosition() > 3000)
      pid[i]->pidReset(pid[i]->GetPIDPosition() - 4095);

    // Finally, enable PID control
    pid[i]->SetPIDEnabled(true);
    pid[i]->SetPIDTimed(pid[i]->GetPIDPosition(),
                        1000); // !FIXME What does this instruction do?
  }

  /*
   * ======= PART 2b: Initialize HID communication =============================
   * In this section we instatiate objects that handle the communication between
   * this firmware and MATLAB. Each of thess objects implements a server that
   * responds to commands sent over HID. During RBE3001, you will be asked to
   * implement your own communication servers. To instantiate a new server, use
   * the template below:
   *
   ...
   * (see Part 2b of source file). The `event()' method implements
   * upstream/downstream communication between this firmware and MATLAB - for
   * more information on this topic, see the example provided in
   * /src/coms/PidServer.cpp.
   *
   * IMPORTANT: the code below includes print statements that, while being
   * useful for debugging/testing, will slow down the main loop execution rate.
   *            You can disable these statements at compile time, by commenting
   * out the DEBUG macro at the beginning of this source file.
   */
